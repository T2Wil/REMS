...
import {getCourseMembers} from '../redux/actions/coursesActions';
import {getSomeFieldsOfObjects} from '../helpers/utils';
...
const Members = (props) => {
  ...
  useEffect(()=> {
     props.getStudents();
     props.getCourseMembers(localStorage.getItem('courseId'));
  },[])
  // const addAssignment = () => {
  ...
  // await props.getCourseMembers(localStorage.getItem('courseId'));
  ...
  // getSomeFieldsOfObjects()
  const tableData = getSomeFieldsOfObjects(props.members,['fullName','studentUniqueNumber','role','section','status']) ;
  return (
    <SpecificCourse page={`${localStorage.getItem('courseName')} > Members`} submenu="Members" >
      ...
      <Table tableHeaders={['name','S.U.N','role','section']} tableRows={[['William','2015','student','JSF intro','Pending','---']]} />
...
const mapDispatchToProps = (dispatch)=> {
  return {
    ...
    getCourseMembers:(courseId) => dispatch(getCourseMembers(courseId))
  }
}
